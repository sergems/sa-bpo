  ...
  return (
    <main className="site-shell">
      ...
      <section id="top" className="hero-section">
        ...
        <div className="hero-copy"><p className="eyebrow"><i /> People-powered operations</p><h1>Our people<br /><em>speak for</em><br />your brand.</h1><p>The conversations behind your growth, delivered with local fluency, global standards, and enterprise discipline.</p><div className="hero-actions"><button className="button button--green" onClick={() => goTo("contact")}>Start a conversation <ArrowRight size={17} /></button><button className="text-button" onClick={() => goTo("about")}>Explore SA-BPO <ArrowDownRight size={16} /></button></div></div>
        <div className="hero-image"><div className="hero-orbit hero-orbit--a" /><div className="hero-orbit hero-orbit--b" /><img src="/manus-storage/sabpo-hero-team_ffe3234b.png" alt="SA-BPO customer experience team" /><div className="hero-badge"><span>01</span><b>Human signal</b></div></div>
      ...
      <section className="calculator-section"><div><p className="eyebrow">A practical starting point</p><h2>Map the work.<br /><em>See the signal.</em></h2></div><div className="calculator-path"><span><b>01</b> Volume</span><i>→</i><span><b>02</b> People</span><i>→</i><span><b>03</b> Outcome</span></div><div><p>Shape an indicative operating model from the work, people, and coverage you need.</p><button className="button button--yellow" onClick={() => setShowCalculator(true)}>Open BPO calculator <ArrowDownRight size={17} /></button></div></section>
      ...
      {showCalculator && <div className="modal-backdrop" onClick={() => setShowCalculator(false)}><div className="calculator-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setShowCalculator(false)} aria-label="Close calculator"><X /></button><p className="modal-kicker"><Sparkles size={15} /> BPO calculator</p><h2>Shape the first<br /><em>conversation.</em></h2><p>Use the inputs to create a simple coverage signal. A full operating model is always tailored with you.</p><label>Specialists <output>{agents}</output><input type="range" min="2" max="40" value={agents} onChange={(event) => setAgents(Number(event.target.value))} /></label><label>Hours per specialist / month <output>{hours}</output><input type="range" min="80" max="220" step="10" value={hours} onChange={(event) => setHours(Number(event.target.value))} /></label><div className="coverage-output"><span>Indicative specialist hours</span><strong>{coverageHours.toLocaleString()}</strong><small>Coverage signal across voice, email, and chat.</small></div><button className="button button--green button--full" onClick={() => { setShowCalculator(false); goTo("contact"); }}>Use this as a starting point <ArrowRight size={16} /></button></div></div>}
    </main>
  );
}
